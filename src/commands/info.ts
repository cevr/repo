...
export const info = Command.make("info", { spec: specArg, json: jsonOption }, ({ spec, json }) =>
  Effect.gen(function* () {
    ...
    const currentRef = isGit
      ? yield* git.getCurrentRef(existing.path).pipe(Effect.orElseSucceed(() => "unknown"))
      : null;
    ...
    if (json) {
      ...
      yield* Console.log(`Fetched:  ${formatRelativeTime(new Date(existing.fetchedAt))}`);
      yield* Console.log(`Accessed: ${formatRelativeTime(new Date(existing.lastAccessedAt))}`);
